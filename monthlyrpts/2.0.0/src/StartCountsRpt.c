/**
@file  StartCountsRpt.c
@brief Job Start Counts per day Report
*/
...
/**
 * @brief   Job Start Count Report
 * @details Produce Job Start Count Report
...
int	mdom;
	...
	if (Flags & PR_DATA) {
		...
		sprintf(rptname,"%s/StartCounts.dat",ReportDir);
		/*
		 ...
		 *  Column headers
		 */
		fprintf(fd,"#Number of jobs started:\n");
		...
		for (x=1;x<=dom;x++) {
			fprintf(fd,
				...
				x,counts[0][x],counts[1][x],counts[2][x],counts[3][x],
			        counts[4][x],counts[5][x],counts[6][x],counts[7][x],
			        counts[8][x],counts[9][x],counts[10][x]);
		}
	...
	if (Flags & PR_PLOT) {
		...
		while (PlotFormats[x] != NULL) {
			...
			bzero(rptname,sizeof(rptname));
			sprintf(rptname,
				"module load ploticus; pl -o %s/StartCounts.%s -%s %s/StartCounts.ploticus DATAFILE=%s/StartCounts.dat DIM=%d MONTH=%s YEAR=%d",
				ReportDir,PlotFormats[x],PlotFormats[x],PLOTDIR,ReportDir,dom,MONTHS[month+1],year);
			system(rptname);

			bzero(rptname,sizeof(rptname));
			sprintf(rptname,"%s/StartCounts.%s",ReportDir,PlotFormats[x]);
