/**
@file  SubmitCountsRpt.c
@brief Submit Counts per day Report
*/
...
/**
 * @brief   Job Submit Count Report
 * @details Produce Job Submit Count Report
...
int	mdom;
	...
	if (Flags & PR_DATA) {
		...
		sprintf(rptname,"%s/SubmitCounts.dat",ReportDir);
		...
		*/
		 ...
		 *  Column headers
		 */
		fprintf(fd,"#Number of jobs submitted:\n");
	...
	if (Flags & PR_PLOT) {
		...
		while (PlotFormats[x] != NULL) {
			...
			bzero(rptname,sizeof(rptname));
			sprintf(rptname,
				"module load ploticus; pl -o %s/SubmitCounts.%s -%s %s/SubmitCounts.ploticus DATAFILE=%s/SubmitCounts.dat DIM=%d MONTH=%s YEAR=%d",
				ReportDir,PlotFormats[x],PlotFormats[x],PLOTDIR,ReportDir,dom,MONTHS[month+1],year);
			system(rptname);

			bzero(rptname,sizeof(rptname));
			sprintf(rptname,"%s/SubmitCounts.%s",ReportDir,PlotFormats[x]);
