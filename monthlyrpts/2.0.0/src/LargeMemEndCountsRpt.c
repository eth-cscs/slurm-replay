/**
@file  LargeMemEndCountsRpt.c
@brief Large Memory Job End Counts per day
*/
...
/**
 * @brief   Large Memory Job End Counts
 * @details Produce Large Memory Job End Counts Report
...
int	mdom;
	...
	if (Flags & PR_DATA) {
		...
		while ((jte=GetJobTableEntry()) != NULL) {
			...
			mdom = (jte->jte_end-tms)/86400 + 1;	/* what day of the month did the job end */
		...
		sprintf(rptname,"%s/LargeMemEndCounts.dat",ReportDir);
		 ...
		 *  Column headings
		 */
		fprintf(fd,"#Number of large memory jobs ending:\n");
	...
	if (Flags & PR_PLOT) {
		...
		while (PlotFormats[x] != NULL) {
			...
			bzero(rptname,sizeof(rptname));
			sprintf(rptname,
				"module load ploticus; pl -o %s/LargeMemEndCounts.%s -%s %s/LargeMemEndCounts.ploticus DATAFILE=%s/LargeMemEndCounts.dat DIM=%d MONTH=%s YEAR=%d",
				ReportDir,PlotFormats[x],PlotFormats[x],PLOTDIR,ReportDir,dom,MONTHS[month+1],year);
			system(rptname);

			bzero(rptname,sizeof(rptname));
			sprintf(rptname,"%s/LargeMemEndCounts.%s",ReportDir,PlotFormats[x]);
