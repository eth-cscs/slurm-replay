/**
@file  EndCountsRpt.c
@brief Job End Counts per day report
*/
...
/**
 * @brief   Job End Count Report
 * @details Produce Job End Count Report
...
int	mdom;
	...
	if (Flags & PR_DATA) {
		...
		sprintf(rptname,"%s/EndCounts.dat",ReportDir);
		/*
	...
	if (Flags & PR_PLOT) {
		...
		while (PlotFormats[x] != NULL) {
			...
			bzero(rptname,sizeof(rptname));
			sprintf(rptname,
				"module load ploticus; pl -o %s/EndCounts.%s -%s %s/EndCounts.ploticus DATAFILE=%s/EndCounts.dat DIM=%d MONTH=%s YEAR=%d",
				ReportDir,PlotFormats[x],PlotFormats[x],PLOTDIR,ReportDir,dom,MONTHS[month+1],year);
			system(rptname);

			bzero(rptname,sizeof(rptname));
			sprintf(rptname,"%s/EndCounts.%s",ReportDir,PlotFormats[x]);
