/**
@file  LargeMemSubmitCountsRpt.c
@brief Large Memory Submit Counts per day Report
*/
...
/**
 * @brief   Large Memory Job Submit Counts
 * @details Produce Large Memory Job Submit Counts per day Report
 * @param   month  Specified month
 * @param   year   Specified year
 * @param   tms    Start timestamp
 * @param   tme    End timestamp
 * @param   dim    Days In Month
 * @param   dom    Days In Month
 */
void 
...
int	mdom;
	...
	if (Flags & PR_DATA) {
		...
		while ((jte=GetJobTableEntry()) != NULL) {
			...
			mdom = (jte->jte_submit-tms)/86400 + 1;	/* what day of the month was it submitted */
			...
			if (tblqueue == 0 ) {
				printf("jobid = %d, mem = %d\n",jte->jte_jobid, jte->jte_mem);
				exit(0);
			}
		...
		sprintf(rptname,"%s/LargeMemSubmitCounts.dat",ReportDir);
		 ...
		 *  Column headings
		 */
		fprintf(fd,"#Number of large memory jobs submitted:\n");
	...
	if (Flags & PR_PLOT) {
		...
		while (PlotFormats[x] != NULL) {
		        ...
		        bzero(rptname,sizeof(rptname));
		        sprintf(rptname,
				"module load ploticus; pl -o %s/LargeMemSubmitCounts.%s -%s %s/LargeMemSubmitCounts.ploticus DATAFILE=%s/LargeMemSubmitCounts.dat DIM=%d MONTH=%s YEAR=%d",
				ReportDir,PlotFormats[x],PlotFormats[x],PLOTDIR,ReportDir,dom,MONTHS[month+1],year);
			system(rptname);

			bzero(rptname,sizeof(rptname));
			sprintf(rptname,"%s/LargeMemSubmitCounts.%s",ReportDir,PlotFormats[x]);
